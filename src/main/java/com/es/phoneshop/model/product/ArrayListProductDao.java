...
public class ArrayListProductDao implements ProductDao {
    ...
    public synchronized Product getProduct(Long id) throws ProductNotFoundException{
        return products.stream()
                .filter(product -> id.equals(product.getId())) //производительнее, но неустойчивей, чем наоборот
                .findAny()
                .orElseThrow(ProductNotFoundException::new); //в get NoSuchElement выбрасывается, надо либо избавиться от get, либо явно указать в сигнатуре, либо создать провеяемое исключение
    }
    ...
    public synchronized void save(Product product) {
        //если у продукта id не нулл, надо обновить соотв продукт
        product.setId(maxId++);
